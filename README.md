...
**最新本地开发：需求已支持按数据批次和版本保存到当前浏览器。** 刷新或重新打开可恢复独立副本，支持删除与恢复原件，保存失败保留草稿并提示。此增量仅本地提交，未 push 或部署；线上仍是上段记录的先前版本。
...
2. 点击 **Client requirements**，由销售填写需求或粘贴沟通内容，运行规则提取，检查并修改结构化条件后应用；也可以选择已有需求并点击 **Review selected requirement** 审核面积口径和原文条件。应用审核会保存同客户的独立浏览器副本，保留原需求和原文；在副本上使用 **Delete local copy** 删除，或 **Restore original** 删除当前副本并返回导入原件，其他副本不受影响。
...
手动筛选与助手最终调用同一个 `filterListings`，使用同一份房源数据。规则助手没有接入大模型；提取结果是销售审核草稿。应用成功后显示 **Saved in this browser · 已保存到当前浏览器**，刷新或重开同一浏览器配置、地址、数据版本后可以再次选用。未应用的表单不保存；刷新重置当前搜索选择。保存不确认业务条件、不写回源文件或 API，清除网站存储会失去本地副本。完整范围和失败处理见 [本机保存说明](docs/local-requirements.md)。
...
- 当前不含 XLSX 直读、需求服务器备份/跨设备同步、真实模型调用、CRM 写回、自动外发、持续采集及物业管理。本机副本按地址、来源和五表内容版本隔离，不自动迁移至新批次。

最新本机保存验证：单元测试 **114/114**、构建通过；新增 Chrome 用例 **6/6**；原有 22 项先通过 21 项，另 1 项点击期间受到 Vite 热更新干扰，代码稳定后原样重跑通过。真实接收批次另完成 **9/9 阶段**，包括同一配置的 Chrome 完全关闭并重开两次，保存前后候选及反向理由一致、恢复原件后删除保持、价格来源可操作。四份原始文件哈希与 49 条 API 原始记录不变，15 条参考仍为 draft。详细日志和限制见开发交接。
...
已知构建提示：最新主 JS 包约 1,054 kB，gzip 332 kB；未做代码拆分。本地交互验证通过，不把此结果当作公网性能验收。以下部署记录为历史结果，本机保存增量未上线。
