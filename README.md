...
**最新本地改版：Home 规则助手、演示销售身份、公私客户目录、面积区间与列头排序、Reports。** 私有需求和看房记录按当前浏览器、数据版本与 Sales ID 保存，支持刷新恢复；原件及人工草稿保留。此增量仅本地开发，未 push 或部署；线上仍是上段记录的先前版本。范围及未定业务事项见 [改版执行约定](docs/iteration-03-implementation.md)。
...
7. 在 **Data & sources** 查看数据性质及隔离提示。**Refresh data** 重新读取当前数据源；本轮本地身份/报告功能未发布到线上。
...
独立项目为 `kwillsaveworld/bhhs-gulf-properties-demo`，已连接上述 GitHub 仓库。此前发布配置为推送到 `main` 触发 Production 构建，访客 SSO、密码和 IP 保护均未启用；本轮没有更改或重新核对公网配置。本地构建使用：
...
已知构建提示：改版主 JS 包约 1,100 kB，gzip 344 kB；未做代码拆分。本地交互验证不代表公网性能验收。以下部署记录为历史结果，本机保存与改版增量均未上线。
