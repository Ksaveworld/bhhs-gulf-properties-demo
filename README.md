...
**当前默认数据全部虚构。** 项目包含 10 个房源、11 条挂牌快照、6 条交易、7 条关联、6 份客户需求和 8 条匹配参考。价格、客户陈述、房屋身份和来源都是交互样例，不代表 BHHS 事实或产品已确认规则。
...
3. 点击候选房源打开详情，查看原币种挂牌价、来源、更新时间、挂牌快照，以及分别展示的 **Same-property history** 和可比成交。
4. 在房源详情查看潜在客户的满足条件、冲突、Unknown、预算适配、购买时间及下一步建议；在 **Clients & needs** 中反向查看一位客户的候选房源。
...
主代理在当前实现上实际执行：`npm test` **33/33 通过**，`npm run build` **通过**，Chrome 浏览器主链路 **9/9 通过**。覆盖预算/地区变化、普通与助手同源结果、详情/单位/价格证据、双向客户匹配、无历史/缺字段/空数据/无匹配、错误重试和 1366×768 桌面操作。最终业务验收仍由控制塔结合产品 A/B 输入完成。

已知构建提示：主 JS 包约 1,010 kB，gzip 318 kB；首版未做代码拆分。本地交互验证通过，不把此结果当作公网性能验收。
