...
export function App() {
    ...
    const [demoProfiles, setDemoProfiles] = useState<DemoProfile[]>(() => [khalidProfile(), ...closedClientProfiles()]);
    ...
    function changeIdentity(next: SalesIdentity | null) {
        ...
        if (previous && previous.sales_id !== next?.sales_id) {
            setClientProfiles({}); setDemoProfiles([khalidProfile(), ...closedClientProfiles()]); setHomeVersion(v => v + 1); setHomeTask('property'); setCreateOpen(false);
    ...
    function visibility(id: string) { const demo = demoProfiles.find(p => `${p.id}-REQ` === id); if (demo) return demo.fixture || demo.closedCaseId ? 'company' as const : 'private' as const; const req = requirements.find(r => r.requirement_id === id) ?? localById.get(id)?.requirement; if (req && dataset?.client_requirements.some(r => r.client_id === req.client_id))
    ...
    <div className="workspace"><aside className="sidebar"><div className="brand"><span className="brand-circle">BHHS</span><span className="brand-small">BERKSHIRE HATHAWAY<br />HOMESERVICES</span><strong>Gulf Properties</strong></div><div className="workspace-label">SALES WORKSPACE</div>
      ...
      <main hidden={route.details.some(d => d.kind === 'client')} className={`main-content ${route.page === 'home' ? 'prototype-home-main' : ''}`}>{route.page === 'properties' && <div className="page-heading"><div><p className="eyebrow">BHHS GULF PROPERTIES</p><h1>{headings[route.page][0]}</h1><p className="heading-description">{headings[route.page][1]}</p></div></div>}
