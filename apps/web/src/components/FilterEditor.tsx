...
export function FilterEditor({ value, onChange, areas, compact = false, requirementsMode = false }: Props) {
  ...
  return <div className={`filter-grid ${compact ? 'compact' : ''}`}>
    ...
    <label className="field"><span>Area unit</span><Select aria-label="Area unit" value={value.area_unit} options={options(['sqft', 'sqm'])} onChange={v => set('area_unit', v)} /></label>
    <label className="field"><span>Area basis</span><Select aria-label="Area basis" value={value.area_basis} options={options(['built_up', 'internal', 'gross', 'land', 'unknown'])} onChange={v => set('area_basis', v)} /></label>
