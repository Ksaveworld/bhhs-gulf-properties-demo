...
function SourceReference({ value }: { value: string | null | undefined }) {
  ...
  try { const url = new URL(value || ''); if (url.protocol === 'http:' || url.protocol === 'https:') safeUrl = url.href; } catch { /* Opaque evidence references are retained in data, not turned into links. */ }
  return safeUrl ? <a href={safeUrl} target="_blank" rel="noopener noreferrer" className="pd-source-ref">View Source</a> : <span className="pd-muted">Not linked</span>;
}
function SourceDetails({ source }: { source: SourceRecord }) {
  return <Descriptions className="pd-descriptions" size="small" column={{ xs: 1, sm: 2 }} layout="vertical">
    <Descriptions.Item label="Source"><span className="pd-source"><span>{source.data_kind === 'demo' ? 'Sample source' : source.source_name || 'Not supplied'}</span><SourceReference value={source.source_ref} /></span></Descriptions.Item>
...
function SalesConfirmation({ listing, salesId, scope, onSignIn }: { listing: ListingSnapshot; salesId: string | null; scope: string | null; onSignIn?: () => void }) {
  ...
  return <div className="pd-sales-confirmation">
    <Checkbox checked={!!current} disabled={!!salesId && (!key || loadedKey !== key)} onChange={event => confirm(event.target.checked)}>Reviewed by local sales</Checkbox>
    {current && <dl data-testid="listing-confirmation"><div><dt>Confirmed At</dt><dd>{date(current.confirmed_at)}</dd></div><div><dt>Confirmed By Sales ID</dt><dd>{current.confirmed_by_sales_id}</dd></div></dl>}
...
function Overview({ listing, salesId, scope, onSignIn }: { listing: ListingSnapshot; salesId: string | null; scope: string | null; onSignIn?: () => void }) {
  const id = useId();
  ...
  return <div className="pd-tab-content">
    ...
    <section className="pd-section" aria-labelledby={id + '-property-facts'}><h3 id={id + '-property-facts'}>Property Facts</h3><div className="pd-fact-strip">
...
function TransactionEvidence({ record, comparable = false }: { record: LinkedTransaction; comparable?: boolean }) {
  ...
  return <article className="pd-transaction" aria-label={(comparable ? 'Comparable transaction ' : 'Same-property transaction ') + transaction.transaction_id}>
    ...
    <div className="pd-transaction-context">{[transaction.area_name, transaction.building_name ? propertyDisplayName({ ...transaction, title: '' }) : null].filter(Boolean).join(' · ')} · {bedrooms(transaction.bedrooms)} · {area(transaction.area_value, transaction.area_unit)} · {label(transaction.area_basis)}</div>
    <Descriptions className="pd-descriptions" size="small" column={{ xs: 1, sm: 2 }} layout="vertical"><Descriptions.Item label="Transaction Type">{label(transaction.record_type)} · {label(transaction.transaction_scope)}</Descriptions.Item><Descriptions.Item label="Transaction ID">{transaction.transaction_id}</Descriptions.Item>{transaction.unit_ref && <Descriptions.Item label="Unit Reference">{transaction.unit_ref}</Descriptions.Item>}</Descriptions>
...
export function PriceEvidence({ listing, dataset }: { listing: ListingSnapshot; dataset: Dataset }) {
  ...
  return <div className="pd-tab-content">
    <section className="pd-section pd-history-section" aria-labelledby={id + '-same-property-history'}><div className="pd-section-heading"><h3 id={id + '-same-property-history'}>Property Transaction History</h3><Tag>{history.length} records</Tag></div>
      <TransactionHistory key={listing.snapshot_id} records={history} renderRecord={record => <TransactionEvidence record={record} />} />
    </section>
    <section className="pd-section pd-comparable-section" aria-labelledby={id + '-comparable-transactions'}><div className="pd-section-heading"><h3 id={id + '-comparable-transactions'}>Comparable Property Transactions</h3><Tag>{comparables.length} records</Tag></div><p className="pd-field-note">Other properties. Review the comparison basis and recorded differences.</p>
...
function ClientCard({ client, listing, onViewClient, viewingRecords }: { client: ClientGroup; listing: ListingSnapshot; onViewClient: PropertyDetailProps['onViewClient']; viewingRecords: ViewingRecord[] }) {
  ...
  return <article className={'pd-client pd-client-' + client.status} aria-label={'Client match for ' + clientDisplayName(requirement)} data-client-id={client.client_id}>
    <header className="pd-client-heading"><div><h3>{clientDisplayName(requirement)}</h3><span className="pd-muted">{client.client_id}</span></div><Button className="pd-view-client" onClick={() => onViewClient(requirement)}>View Client Details</Button></header>
    ...
    <footer className="pd-client-next"><p><strong>Next Action: </strong>{question ? 'Confirm this point with the client before recommending a viewing.' : 'Confirm current availability and discuss a viewing with the client.'}</p></footer>
...
function PotentialClients({ listing, requirements, onViewClient, viewingRecords = [] }: Pick<PropertyDetailProps, 'requirements' | 'onViewClient' | 'viewingRecords'> & { listing: ListingSnapshot }) {
  ...
  return <div className="pd-tab-content">{(['match', 'review'] as const).map(status => {
    ...
    return <section className={'pd-client-group pd-client-group-' + status} key={status} aria-label={status === 'match' ? 'Customers with conditions met' : 'Customers needing clarification'}><h3>{status === 'match' ? 'Condition Met' : 'Needs Clarification'} ({group.length})</h3>{group.length ? group.map(client => <ClientCard key={client.client_id} client={client} listing={listing} onViewClient={onViewClient} viewingRecords={viewingRecords} />) : <p className="pd-field-note">No clients in this group.</p>}</section>;
  })}</div>;
}
export function PropertyDetail({ listing, dataset, requirements, onClose, onViewClient, salesId = null, storageScope = null, viewingRecords = [], open, onExport, onSignIn }: PropertyDetailProps) {
