...
import { CaretDownOutlined, CaretUpOutlined, CheckCircleOutlined, FilterOutlined, InfoCircleOutlined, SlidersOutlined } from '@ant-design/icons';
...
type Props = {
  listings: ListingSnapshot[]; filters: Filters; onFilter: (next: Filters) => void;
  active: ClientRequirement | null;
  onOpen: (id: string) => void; onReset: () => void;
};
export function PropertyLibrary({ listings, filters, onFilter, active, onOpen, onReset }: Props) {
  ...
  const dateError = Boolean(filters.move_in_by && !isValidEnglishDateValue(filters.move_in_by));
  const rows = budgetError || dateError ? [] : filterListings(listings, filters);
  ...
  const columns: ColumnsType<ListingSnapshot> = [
    { title: 'Property', key: 'property', width: 195, render: (_, l) => <button className="property-link" onClick={() => onOpen(l.listing_id)} aria-label={`Open ${propertyDisplayName(l)}`}><strong>{propertyDisplayName(l)}</strong></button> },
    ...
    { title: sortTitle('Asking price', 'price'), key: 'price', width: 150, render: (_, l) => <div><strong className="price">{money(l.asking_price, l.currency)}</strong><span className="table-caption">{l.listing_status === 'active' ? 'Current listing' : 'Recorded asking price'}</span></div> },
    ...
    { title: sortTitle('Updated', 'updated'), key: 'updated', width: 120, render: (_, l) => <span className="updated-date">{date(l.captured_at)}</span> },
  ...
  return <>
    <section className="filter-panel" aria-label="Property filters">
      ...
      <div className="pv2-filters">
        ...
        {expanded && <>
          ...
          <label className="field"><span>Available / Move-in By</span><EnglishDateInput label="Available by" value={filters.move_in_by ?? ''} onChange={value => set('move_in_by', value || null)} /></label>
        </>}
      </div>
    ...
    <div className="pv2-library-layout">
      <section className="results-panel"><div className="results-toolbar"><div><h2><span data-testid="result-count">{rows.length}</span> properties</h2><span>{active ? `Structured search for ${clientDisplayName(active)}` : 'In your current selection'}</span></div><span className="source-note">Sort using the column arrows</span></div>
        <Table className="property-table" rowKey="listing_id" columns={columns} dataSource={rows} pagination={{ current: page, onChange: setPage, pageSize: 6, showSizeChanger: false, hideOnSinglePage: true }} scroll={{ x: active ? 1160 : 1035 }} onRow={l => ({ 'data-testid': `listing-${l.listing_id}` } as HTMLAttributes<HTMLTableRowElement>)} locale={{ emptyText: <Empty description={<><strong>{dateError ? 'Correct the availability date to see results.' : areaPending ? 'Area comparison is awaiting confirmation.' : 'No properties meet these filters.'}</strong><p>{areaPending ? 'Missing information does not confirm a lack of suitable properties.' : 'Review your filters. Unknown fields cannot satisfy a selected condition.'}</p><Button onClick={onReset}>Reset filters</Button></>} /> }} />
