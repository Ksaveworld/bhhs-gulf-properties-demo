...
export function PrototypeHome({ profiles, onChange, onOpen, quickTools }: Props) {
  ...
  function confirmIdentity(next: DemoProfile, label: string) { setDraft(next); setIdentity(label); setStage(3); }
  function create() {
  ...
  function finish(value: string) {
    ...
    const next: DemoProfile = { ...draft, payment: value, paymentSource: value ? source : '', updated: 'Just now',
      core: draft.core.map(f => f.key === 'budget' && choices.budget === 'crm' ? { ...f, value: 'Up to AED 16m', sources: ['7'] } : f.key === 'home' && choices.home === 'crm' ? { ...f, value: 'Apartment · 4 bedrooms', sources: ['7'] } : f),
      known: draft.known.map(f => f.key === 'budget-note' && choices.budget === 'crm' ? { ...f, value: 'CRM ceiling retained: AED 16m. Material proposes AED 18–22m.', sources: ['7', '1'] } : f.key === 'what' && choices.home === 'crm' ? { ...f, value: 'CRM apartment brief retained; the proposed villa preference was not adopted.', sources: ['7', '4'] } : f),
      sources: [...draft.sources, { id: source, title: 'Sales confirmation', meta: 'Just now', text: `${identity}\nBudget: ${choices.budget === 'new' ? 'Adopt new information: AED 18–22m' : 'Keep CRM: AED 16m'}\nProperty type: ${choices.home === 'new' ? 'Adopt new information: villa' : 'Keep CRM: apartment'}\nPayment: ${value || 'Confirm later in the call'}${notes ? `\nSubmitted sales note (not part of the preset extraction): ${notes}` : ''}` }],
    };
