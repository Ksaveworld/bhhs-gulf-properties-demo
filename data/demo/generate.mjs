...
const requirements = [
  ...
  requirement('007', { client_id: 'DEMO-C-001', client_alias: 'Demo client A · Marina home', raw_request: 'Demo second requirement from client A: a ready Dubai Marina apartment with at least 2 bedrooms and parking, up to AED 3m. Purchase by 2026-12-01, move in by 2027-01-01. This is a separate scenario; keep the earlier requirement.', budget_max: 3000000, currency: 'AED', budget_constraint: 'hard', preferred_areas: ['Dubai Marina'], bedrooms_min: 2, purchase_purpose: 'self_use', market_preference: 'ready', purchase_by: '2026-12-01', move_in_by: '2027-01-01', hard_constraints: 'must have parking', missing_questions: null, intent_evidence: 'Synthetic client requested a viewing for this separate shortlist.', notes: `${syntheticNote} Second independent requirement for DEMO-C-001; do not combine budgets or conditions with DEMO-R-001.` }),
  requirement('008', { client_id: 'DEMO-C-002', client_alias: 'Demo client B · Marina shortlist', raw_request: 'Demo second requirement from client B: a ready two-bedroom apartment in Dubai Marina, at most AED 2.9m. Purchase by 2026-10-15 and move in by 2026-11-01. Keep this separate from the earlier AED 2.6m requirement.', budget_max: 2900000, currency: 'AED', budget_constraint: 'hard', preferred_areas: ['Dubai Marina'], bedrooms_min: 2, purchase_purpose: 'self_use', market_preference: 'ready', purchase_by: '2026-10-15', move_in_by: '2026-11-01', missing_questions: null, intent_evidence: 'Synthetic client asked for a viewing before reviewing an offer.', notes: `${syntheticNote} Second independent requirement for DEMO-C-002; the original requirement still conflicts with DEMO-L-002.` }),
];
...
console.log(`Created synthetic BHHS interaction fixtures: 10 listings / ${listings.length} snapshots, ${transactions.length} transactions, ${links.length} links, ${requirements.length} requirements across ${new Set(requirements.map(row => row.client_id)).size} clients, ${references.length} references.`);
