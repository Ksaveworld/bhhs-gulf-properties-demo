...
import { clientDisplayName, propertyAreaSqft, propertyDisplayName } from './property-presentation';
...
export function propertySalesReport(listing: ListingSnapshot, dataset: Dataset, requirements: ClientRequirement[], viewingRecords: ViewingRecord[] = []): SalesReport {
    ...
    const sections: ReportSection[] = [
        ...
        { heading: 'Comparable Property Transactions', lines: comparables.length ? comparables.map(({ transaction: t, link }) => `${t.transaction_date} · ${display(t.date_basis)} date · ${amount(t.amount, t.currency)} · ${propertyDisplayName({ ...t, title: '' })}\nComparison: ${link.match_basis}\nDifferences: ${display(link.differences)}\nSource: ${t.source_ref}`) : ['No eligible comparable transactions.'] },
    ];
...
export function clientSalesReport(clientId: string, requirements: ClientRequirement[], originals: ClientRequirement[], copies: LocalRequirementCopy[], listings: ListingSnapshot[], viewings: ViewingRecord[], ownership: string, selectedRequirementId?: string, viewingListings: ListingSnapshot[] = listings): SalesReport {
    ...
    const sections: ReportSection[] = [{ heading: 'Client details', lines: [`Client: ${clientDisplayName(plans[0])}`, `Client ID: ${clientId}`, `Ownership: ${ownership}`] }];
    plans.forEach((req, i) => {
    ...
    return { title: clientDisplayName(plans[0]), subtitle: 'Client sales brief', disclosure: notice(plans.some(r => r.data_kind === 'demo') || listings.some(l => l.data_kind === 'demo')), sections, filename: filename('Client', clientId) };
}
