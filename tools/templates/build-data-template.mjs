...
const outputDir = path.join(root, 'outputs/01a06522-fb17-7d92-9e65-14eaf803b75e');
const qaDir = path.join(root, '.work/template-qa');
...
title(guide, 'BHHS｜产品数据交付模板 v1', '产品 A：房源、成交与关联证据　／　产品 B：客户需求与匹配验收。先填写房源挂牌和客户需求，再补成交关联及人工参考。模板不含真实客户、房源或交易记录。');
...
guide.getRange('A26:H26').merge(); guide.getRange('A26').values = [['依据：用户确认的 BHHS Demo 主链路及分工；字段细化为本次接入模板设计，非客户已验收规则。']];
guide.getRange('A26:H26').format = { font: { color: color.gray }, wrapText: true, rowHeight: 34 };
guide.getRange('A28:B28').merge(); guide.getRange('A28').values = [['功能讨论逐字稿']]; guide.getRange('C28:H28').merge(); guide.getRange('C28').values = [['https://xqmqf98k8t.feishu.cn/docx/UjbfdecT0oSpTBxlz3mcxHdanpd']];
guide.getRange('A29:B29').merge(); guide.getRange('A29').values = [['主会逐字稿']]; guide.getRange('C29:H29').merge(); guide.getRange('C29').values = [['https://xqmqf98k8t.feishu.cn/docx/GFwidM0aVo4XjmxnPjAcqe1gngc']];
guide.getRange('A28:H29').format = { font: { color: color.gray }, wrapText: true, rowHeight: 32 };
...
dict.getRange('A1:I1').merge(); dict.getRange('A1').values = [['BHHS｜字段字典 · v1.0.0']];
...
await fs.writeFile(path.join(root, 'data/templates/README.md'), readme, 'utf8');
...
await fs.writeFile(path.join(root, 'docs/data-contract.md'), contract, 'utf8');
...
for (const [sheetName, range, filename] of [
  ...
  ['00_填写说明', 'A17:H29', '00-guide-bottom.png'],
...
await (await SpreadsheetFile.exportXlsx(wb)).save(path.join(outputDir, 'BHHS_数据字段模板_v1.xlsx'));
...
console.log(JSON.stringify({ output: path.join(outputDir, 'BHHS_数据字段模板_v1.xlsx'), fieldCount: dictionaryRows.length, previews }));
